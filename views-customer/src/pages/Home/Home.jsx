...
function Home() {
    ...
    const fetchMenuItems = async () => {
        try {
            const response = await fetch(`${backendUrl}/products/branch?Branch=${selectedBranch}`, 
            {
                credentials: 'include',
                headers: {
    ...
    return (
        <>
            ...
            <div className='h-screen' style={{ backgroundColor: '#1C5739', minHeight: '150vh' }}>
                ...
                {cartItems.length > 0 && (
                    <div style={{ position: 'fixed', bottom: '17px', left: '10px', right: '10px', padding: '12px', borderRadius: '20px', backgroundColor: '#368D61' }}>
                        <button onClick={handleLatestButtonClick} style={{ display: 'flex', alignItems: 'center',border: 'none', borderRadius: '20px'}}>
